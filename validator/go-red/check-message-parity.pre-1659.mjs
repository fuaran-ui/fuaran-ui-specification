#!/usr/bin/env node
// FROZEN ARTEFACT — the message-parity checker AS IT STOOD BEFORE Phase 1659,
// kept so the go-red proof can show that the fixture the current gate REFUSES is
// one this gate ACCEPTED. Do not maintain it, do not re-sync it with the checker
// beside it, and do not fix anything in it: its whole value is that it is the old
// behaviour, and a copy that has been kept up to date proves nothing.
//
// Verbatim copy of `validator/check-message-parity.mjs` at
// wire-format-fixtures@6e348db8967b4c0c9fe32df63294a66945639019, with exactly two
// mechanical edits, both of them about WHERE it looks and neither about WHAT it
// decides:
//
//   1. `here` is one directory deeper than the original's, so the contract path
//      resolves through `..` and the workspace through three;
//   2. `--workspace <dir>` was added, because the self-test must point it at a
//      fixture host rather than at a real checkout.
//
// The exemption arm below — `messageForm: 'structured'` believed on sight, and
// `fuaran-ts`'s extractor a regex that structurally cannot match — is untouched,
// and is the behaviour under proof.
//
...
// EDIT 1 of 2 (see the header): this file sits one directory deeper than the
// original, so the contract is one `..` away and the workspace three.
const here = dirname(fileURLToPath(import.meta.url));
const contractDir = resolve(here, '..');
// EDIT 2 of 2 (see the header): `--workspace <dir>` for the self-test.
const wsFlag = process.argv.indexOf('--workspace');
const workspace = wsFlag === -1 ? resolve(here, '..', '..', '..') : resolve(process.argv[wsFlag + 1] ?? '.');
const verbose = process.argv.includes('--verbose');

const parity = JSON.parse(readFileSync(join(contractDir, 'message-parity.json'), 'utf8'));
