#!/usr/bin/env node
// FROZEN COPY — do not edit, do not "improve".
//
// Verbatim copy of `validator/check-coverage.mjs` at wire-format-fixtures@339ef9f,
// the last commit before Phase 1692 added the CITATION arm. It exists for one
// assertion in `validator/check-coverage-selftest.mjs`: the stale-citation
// fixture PASSES here and is REFUSED by the live checker, which is what makes
// the refusal a proof about the change rather than about the fixture.
//
// It resolves the vocabulary through `../defect-vocabulary.json` because it sits
// one directory deeper than the file it copies.
