...
`CellKindErased` / `CellFormat` / `ColumnWidth` (grid columns), `Binding<'T>`,
...
- **Shape.** DU positions encode as `oneOf` of branch objects, each pinned by a `$type` `const` discriminator (an unrecognised `$type` matches no branch – mirroring `UNKNOWN_DU_CASE` / `WRONG_NODE_KIND`). Bare-string enums (§3.5) encode as `{ "type":"string", "enum":[…] }`. Closure slots (§4) are the `const "<closure>"`.

  **`Binding<'T>` carries one `$def` per instantiated element type, and each states whether its `Static` payload is required.** A slot points at `Binding_str` / `Binding_float` / `Binding_int` / `Binding_bool` / `Binding_list_SelectOption` / … rather than at one shared type-erased definition, so a boolean at `Metric.trend` or a §7 sentinel at the integer `Stepper.activeStep` is refused by the schema and not only by the decoder. Two element types stay `true` (any JSON) and that is **abstention, not erasure**: a structured JSON payload position (rule 12) and a HOSTED row feed carry content the encoder deliberately does not decompose (§5) – any JSON except `null`, which the decoder rejects. The second axis is **presence**: the four scalar instantiations put `value` in the `Static` arm's `required` list, because absence routes through the slot's own parser (§2 rule 4 — absence is structural) and a scalar parser has no reading for it. The collection instantiations and the two abstentions leave it optional, because theirs normalise absence to the empty payload. `Binding_str_choice` is the one place the element type does not decide it: `Choice.value`, `SegmentedChoice.value`, `Combobox.value` and `Select.value` are `Binding<string>` whose **absent `Static` payload IS the value** ("no selection"), so they carry their own definition with `value` optional and a `description` saying so, and every other string-valued slot keeps the required form. One definition could not state both contracts – which is why this is a split rather than a tightening.
...
([`validator/defect-vocabulary.json`](validator/defect-vocabulary.json), code `FUARAN152`), which
